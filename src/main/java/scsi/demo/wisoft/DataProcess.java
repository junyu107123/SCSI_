...
public JSONObject getNodeSEQ(String[] typevalue , String[] para , String exceptND) throws IOException, SQLException
{
	...
	try{ 
		...
		qry_Statement += " order by node_type , pos";
		rs0 = queryData(qry_Statement, para , typevalue);
		rs0.last();
		rs0.beforeFirst();
		while (rs0.next()){
			...
			json1.append("node_country",rs0.getString("node_country"));
			json.append("node_info",json1);
		}	
	}catch(Exception e){
		System.out.print(e.toString());
...
public JSONObject getConnectionSEQ(String[] typevalue , String[] para , String[] fieldnamelist , String[] filterlist, String[] filtervalue) throws IOException, SQLException
{
	JSONObject json = new JSONObject();
	ResultSet rs0 = null ;
	try{ 
		int noOfField = fieldnamelist.length ;
		String qry_Statement = "SELECT * , (SELECT b.node_failure FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_failure , (SELECT b.node_failure FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_failure, (SELECT b.lon FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_lon , (SELECT b.lon FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_lon, (SELECT b.lat FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_lat , (SELECT b.lat FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_lat, (SELECT b.node_type FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_type , (SELECT b.node_type FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_type, (SELECT b.rn FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA , (SELECT b.rn FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1') )b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB FROM sc_connection c where (c.del_mark is null or c.del_mark <> '1') GROUP BY CONCAT(connection_A,'_',connection_B)  ORDER BY nidA, nidB";
		rs0 = queryData(qry_Statement, para , typevalue);
		rs0.last();
		rs0.beforeFirst();
		while (rs0.next()){
...
public JSONObject getOneConnectionSEQ(String[] typevalue , String[] para , String[] fieldnamelist , String[] filterlist, String[] filtervalue) throws IOException, SQLException
{
	JSONObject json = new JSONObject();
	ResultSet rs0 = null ;
	try{ 
		int noOfField = fieldnamelist.length ;
		String qry_Statement = "SELECT * , (SELECT b.node_failure FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_failure , (SELECT b.node_failure FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_failure, (SELECT b.lon FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_lon , (SELECT b.lon FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_lon, (SELECT b.lat FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_lat , (SELECT b.lat FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_lat, (SELECT b.node_type FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_type , (SELECT b.node_type FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_type, (SELECT b.rn FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA , (SELECT b.rn FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1') )b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB FROM sc_connection c LEFT JOIN sc_connectionfail y ON c.connection_id = y.connectionid AND y.failceased <> 1 where (c.del_mark is null or c.del_mark <> '1') and c.connection_id = ?  GROUP BY CONCAT(connection_A,'_',connection_B)  ORDER BY nidA, nidB";
		rs0 = queryData(qry_Statement, para , typevalue);
		rs0.last();
		rs0.beforeFirst();
		while (rs0.next()){
...
public JSONObject getConnectionFailSEQ(String[] typevalue , String[] para , String[] fieldnamelist , String[] filterlist, String[] filtervalue) throws IOException, SQLException
{
	JSONObject json = new JSONObject();
	ResultSet rs0 = null ;
	try{ 
		int noOfField = fieldnamelist.length ;
		String qry_Statement = "SELECT * , (SELECT b.node_failure FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_failure , (SELECT b.node_failure FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_failure, (SELECT b.lon FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_lon , (SELECT b.lon FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_lon, (SELECT b.lat FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_lat , (SELECT b.lat FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_lat, (SELECT b.node_type FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA_type , (SELECT b.node_type FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB_type, (SELECT b.rn FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1'))b ON b.node_id = a.node_id WHERE c.connection_A = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidA , (SELECT b.rn FROM sc_nodes a LEFT JOIN (SELECT * , ROW_NUMBER() OVER (order by node_id ) as rn  FROM sc_nodes where (del_mark is null or del_mark <> '1') )b ON b.node_id = a.node_id WHERE c.connection_B = a.node_name and (a.del_mark is null or a.del_mark <> '1')) AS nidB FROM sc_connection c LEFT JOIN sc_connectionfail y ON c.connection_id = y.connectionid AND y.failceased <> 1 where (c.del_mark is null or c.del_mark <> '1')  GROUP BY CONCAT(connection_A,'_',connection_B)  ORDER BY nidA, nidB";
		rs0 = queryData(qry_Statement, para , typevalue);
		rs0.last();
		rs0.beforeFirst();
		while (rs0.next()){
...
public String updateFailStatus(String[] aaa) throws IOException,JSONException, SQLException{
	String[] trimValue = {"failvoice","failinternet","failleasing_circuit_line","failleasing_circuit_bw","failleasing_ethernet_line","failleasing_ethernet_bw","failleasing_vpn_line","failleasing_vpn_bw","InterVoice","Internet","InterLeasing","Leasing_circuit_line","Leasing_circuit_bw","Leasing_ethernet_line","Leasing_ethernet_bw","Leasing_vpn_line","Leasing_vpn_bw","connectionid"};
	String statements = "insert into sc_connectionfail ("+trimValue[17];
	String rareState = "values ( ? ";
	String valuestype[] = new String[11];
	String values[] = new String[11];
	JSONObject jsb = new JSONObject(aaa[0]);
	valuestype[0]="s";
	values[0]=jsb.getString("f8");
	int failamount = 0 ;
	for (int v=0 ; v < 8 ; v++){
			statements += "," + trimValue[v] ;
			rareState += ", ? ";
			valuestype[v+1]="s";
			String myid = "f"+Integer.toString(v+9);
			values[v+1]=jsb.getString(myid);
			failamount += Integer.parseInt(values[v+1]);
	}
	valuestype[9]="s";
	UUID uuid = UUID.randomUUID();
	values[9]= uuid.toString();
	values[10] = todaytime_hhmmss();
	valuestype[10]="s";
	statements += " , failid , faildatetime ) "+rareState + " , ? , ?)";
	//System.out.println(statements);
	
	
	//String statements = "update sc_nodes set node_failure = ? where sysid = ?";
	//if(st.equals("links")) statements = "update sc_links set failure = ? where sysid = ?";
	//String[] values={st1,st2};
	//String[] valuestype={"s","s"};
	System.out.print(failamount);
		
	String[] para = {values[0]};
	String[] typevalue = {"s"};
	String qry_Statement = "SELECT * from sc_connectionfail where connectionid = ? and failceased = 0 " ;
	ResultSet rs0 = queryData(qry_Statement, para , typevalue);
	rs0.last();
	rs0.beforeFirst();
	String releaseid = "";
	while (rs0.next()){
		releaseid += (releaseid.equals("")?"":",")+rs0.getString("sysid");
	}
	String[] tmpreleaseid = releaseid.split(",");
	String[] qql = {"s","s"} ;
	for (int jj=0 ; jj<tmpreleaseid.length ; jj++){
		String[] paral = {todaytime_hhmmss(),tmpreleaseid[jj]};
		qry_Statement = "update sc_connectionfail set failceased = 1 , ceaseddatetime = ? where sysid = ?";
		updateData(qry_Statement , paral , qql);
		
	}
	if(failamount > 0){
		qry_Statement = "update sc_connection set level = 1 , notes = '\u670d\u52d9\u5f71\u97ff:' , linecolor= '#C14242' where connection_id = ?";
		updateData(qry_Statement , para , typevalue);		
		updateData(statements , values , valuestype);
	}else{
		qry_Statement = "update sc_connection set level = 0 , notes = '\u670d\u52d9\u5c1a\u7121\u5f71\u97ff:' , linecolor= '#0acf24' where connection_id = ?";
		updateData(qry_Statement , para , typevalue);
	}
...
public void updateStatus(String st, String st1, String st2) throws IOException, SQLException{
	String statements = "update sc_nodes set node_failure = ? where sysid = ?";
	if(st.equals("links")) statements = "update sc_links set failure = ? where sysid = ?";
	String[] values={st1,st2};
	String[] valuestype={"s","s"};
...
public String todaytime_hhmmss() throws IOException, SQLException
{
	Calendar rightNow = Calendar.getInstance();
	String nowtime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(rightNow.getTime());
	return nowtime ;
}
